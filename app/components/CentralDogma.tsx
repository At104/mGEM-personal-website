...
const STEPS = [
  {
    tag: "DNA",
    step: "01",
    process: "Stored in the nucleus",
    title: "Design the blueprint",
    body: "Every mGEM season starts with genetic circuits — sequences designed base by base in Wet Lab, with literature reviews and proof-of-concept work on Benchling.",
    color: "#17B6C9",
    glow: "bg-cyan/20",
  },
  {
    ...
    body: "DNA is copied into messenger RNA — the working instructions. Dry Lab models the system mathematically while Wet Lab generates the data that validates each design.",
    color: "#F4B740",
    glow: "bg-amber/20",
  },
  {
    ...
    body: "Ribosomes read mRNA and assemble proteins — molecular machines built to tackle real-world problems, from biosensors to therapeutics, documented for the iGEM Jamboree.",
    color: "#15A06B",
    glow: "bg-leaf/20",
  },
];

/** Mini double-helix SVG — same glowing-node language as the hero 3D helix. */
...
/** Pinned scroll section: central dogma of molecular biology, scrubbed with GSAP. */
export default function CentralDogma() {
  ...
  useGSAP(
    () => {
      ...
      tl.fromTo(".cd-flow-line", { scaleX: 0 }, { scaleX: 1, ease: "none" }, 0)
        .to(".cd-step-0", { autoAlpha: 0 }, 0.26)
        .to(".cd-step-1", { autoAlpha: 1 }, 0.3)
        .to(".cd-vis-dna", { autoAlpha: 0.25, scale: 0.85 }, 0.3)
        .to(".cd-vis-rna", { autoAlpha: 1, scale: 1 }, 0.3)
        .to(".cd-vis-protein", { autoAlpha: 0.2, scale: 0.8 }, 0.3)
        .to(".cd-arrow-1", { autoAlpha: 1 }, 0.32)
        .to(".cd-step-1", { autoAlpha: 0 }, 0.58)
        .to(".cd-step-2", { autoAlpha: 1 }, 0.62)
        .to(".cd-vis-rna", { autoAlpha: 0.25, scale: 0.85 }, 0.62)
        .to(".cd-vis-protein", { autoAlpha: 1, scale: 1 }, 0.62)
        .to(".cd-arrow-2", { autoAlpha: 1 }, 0.64);
    },
    { scope: root }
  );
  ...
  return (
    <section
      ...
      aria-label="The central dogma of molecular biology"
    >
      ...
      <div className="relative mx-auto w-full max-w-7xl px-6 py-12">
        ...
        <div className="mb-10 max-w-2xl">
          ...
          <h2 className="mt-2 font-display text-3xl font-bold sm:text-4xl">The central dogma</h2>
          <p className="mt-3 text-base leading-relaxed text-white/65">
            ...
            {" → "}
            <span className="text-leaf-300">Protein</span>. Here&apos;s how that maps to how mGEM builds a project.
          </p>
        </div>
        ...
        <div className="grid items-center gap-12 lg:grid-cols-2">
          ...
          <div className="relative h-64 sm:h-72">
            {STEPS.map((s, i) => (
              <div
                key={s.tag}
                className={`cd-step-${i} absolute inset-0 flex flex-col justify-center ${i === 0 ? "" : "opacity-0"}`}
              >
                <div className="flex items-center gap-3">
                  <span className="font-mono text-xs text-white/40">{s.step}</span>
                  <span
                    className="rounded-full px-3 py-1 font-mono text-xs font-bold uppercase tracking-[0.3em]"
                    style={{ color: s.color, backgroundColor: `${s.color}22` }}
                  >
                    {s.tag}
                  </span>
                  <span className="font-mono text-[10px] uppercase tracking-wider text-white/40">{s.process}</span>
                </div>
                <h3 className="mt-4 font-display text-3xl font-bold sm:text-4xl lg:text-5xl">{s.title}</h3>
                <p className="mt-4 max-w-md text-base leading-relaxed text-white/70">{s.body}</p>
              </div>
            ))}
          </div>

          {/* Visual flow: DNA → RNA → Protein */}
          <div className="relative mx-auto w-full max-w-lg">
            <div className="cd-flow-line absolute left-[12%] right-[12%] top-1/2 h-0.5 origin-left -translate-y-1/2 scale-x-0 bg-gradient-to-r from-cyan via-amber to-leaf opacity-40" />
            ...
            <div className="relative grid grid-cols-[1fr_auto_1fr_auto_1fr] items-center gap-2 sm:gap-4">
              ...
              <div className="cd-vis-dna relative flex flex-col items-center">
                ...
                <p className="relative text-[10px] text-white/45">Double helix</p>
              </div>
              ...
              <div className="cd-vis-rna relative flex flex-col items-center opacity-20">
                ...
                <p className="relative text-[10px] text-white/45">mRNA message</p>
              </div>
              ...
              <div className="cd-vis-protein relative flex flex-col items-center opacity-20">
                ...
                <p className="relative text-[10px] text-white/45">Functional product</p>
              </div>
            </div>
          </div>
        </div>
