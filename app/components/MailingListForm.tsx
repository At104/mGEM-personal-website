...
const input =
  "w-full rounded-2xl border border-ink/10 bg-paper-warm px-4 py-3 text-sm text-ink placeholder:text-ink-mute focus:border-leaf focus:outline-none focus:ring-2 focus:ring-leaf/20";
...
export default function MailingListForm({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <button
        type="submit"
        className="w-full rounded-full bg-leaf px-6 py-3 text-sm font-semibold text-white transition hover:bg-leaf-deep sm:w-auto"
      >
        Subscribe
      </button>
    </form>
